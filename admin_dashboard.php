...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .sidebar { width: 260px; background: #FFFFFF; border-right: 1px solid #E9EEF3; display: flex; flex-direction: column; position: fixed; left: 0; top: 0; bottom: 0; z-index: 10; padding: 28px 20px; }
  ...
  .logo-icon { background: #3B82F6; width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; color: white; font-size: 18px; font-weight: 700; }
  ...
  .stat-card { background: white; border-radius: 24px; padding: 20px 24px; border: 1px solid #EFF3F8; }
  .stat-card h4 { font-size: 12px; font-weight: 600; text-transform: uppercase; color: #6C7A91; margin-bottom: 8px; }
  .stat-card .number { font-size: 34px; font-weight: 800; color: #0F172A; }
  ...
  .table-card { background: white; border-radius: 24px; border: 1px solid #EFF3F8; overflow: hidden; margin-bottom: 32px; }
  .table-header { padding: 20px 24px; border-bottom: 1px solid #F0F2F5; }
  .table-header h3 { font-size: 16px; font-weight: 700; display: flex; align-items: center; gap: 8px; }
  .table-wrapper { overflow-x: auto; }
  table { width: 100%; border-collapse: collapse; }
  th { text-align: left; padding: 14px 20px; font-size: 11px; font-weight: 600; text-transform: uppercase; color: #6C7A91; background: #FCFDFF; border-bottom: 1px solid #EDF2F7; }
  td { padding: 14px 20px; font-size: 13px; color: #1E293B; border-bottom: 1px solid #F1F5F9; }
  tr:hover td { background: #F8FAFE; }
  .badge-completed { background: #DCFCE7; color: #15803D; padding: 4px 12px; border-radius: 30px; font-size: 11px; font-weight: 600; display: inline-block; }
  .badge-active { background: #FEF3C7; color: #D97706; padding: 4px 12px; border-radius: 30px; font-size: 11px; font-weight: 600; display: inline-block; }
...
<div class="main-content" id="reportContent">
  ...
  <div class="stats-grid">
    <div class="stat-card"><h4>Total Students</h4><div class="number"><?php echo $total_students; ?></div><div class="trend"><i class="fas fa-arrow-up"></i> Registered</div></div>
    <div class="stat-card"><h4>Total Sit-ins</h4><div class="number"><?php echo $total_sitins; ?></div><div class="trend"><i class="fas fa-calendar"></i> All time</div></div>
    <div class="stat-card"><h4>Total Points</h4><div class="number">⭐ <?php echo number_format($total_points); ?></div><div class="trend"><i class="fas fa-star"></i> Earned</div></div>
    <div class="stat-card"><h4>Completion Rate</h4><div class="number"><?php echo $completion_rate; ?>%</div><div class="trend"><i class="fas fa-chart-line"></i> Success rate</div></div>
  </div>
    ...
    <div class="table-card">
      ...
      <div class="table-wrapper">
        <table id="studentsTable">
          <thead>
            <tr><th>ID Number</th><th>Full Name</th><th>Course</th><th>Year Level</th><th>Sessions Left</th><th>Points</th>
          </thead>
          <tbody>
            <?php if ($students_result && $students_result->num_rows > 0): ?>
              ...
              <?php endwhile; ?>
            <?php else: ?>
              <tr><td colspan="6" style="text-align:center; padding:48px;">No student records found</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
    ...
    <div class="table-card">
      ...
      <div class="table-wrapper">
        <table id="sessionsTable">
          <thead>
            <tr><th>Date</th><th>Student Name</th><th>ID Number</th><th>Course</th><th>Lab</th><th>Purpose</th><th>Time In</th><th>Time Out</th><th>Points</th><th>Status</th>
          </thead>
          <tbody>
            <?php if ($sessions_result && $sessions_result->num_rows > 0): ?>
              ...
              <?php endwhile; ?>
            <?php else: ?>
              <tr><td colspan="10" style="text-align:center; padding:48px;">No sit-in records found</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
